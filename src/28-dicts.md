...
# Несуществующий ключ вызовет KeyError: student["grade"]
...
last = d.popitem()   # удаляет и возвращает последнюю добавленную пару (ключ, значение)
d.clear()            # очищает словарь

# Проверка наличия ключа (после clear() словарь пуст)
print("a" in d)  # False
```
...
Ключи словаря должны быть **хешируемыми** (обычно это неизменяемые объекты). Причина та же, что и у элементов множества: словарь находит значение по хешу ключа (см. [урок 26](26-sets.html)), а хеш изменяемого объекта мог бы измениться. Поэтому кортеж можно использовать в качестве ключа, а список — нельзя:
...
Однако **кортеж** может быть ключом только тогда, когда все его элементы хешируемы (неизменяемы). Кортеж со списком внутри тоже не подойдёт:
