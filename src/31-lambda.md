...
Лямбду можно вызвать сразу, без присваивания переменной — для этого оберните её в скобки и передайте аргументы следом:
...
> **💡 Совет:** если логика занимает более одной строки или требует нескольких (тем более вложенных) условий, используйте обычную функцию с `def`. Lambda — для коротких, простых выражений: один тернарный оператор вроде `a if a > b else b` — ещё допустимо, цепочка `x**2 if x > 0 else x**3 if x < 0 else 0` — уже нет.
>
> **📐 PEP 8:** не присваивайте лямбду переменной (`double = lambda x: x * 2`) — для именованных функций всегда используйте `def`. Присваивания в примерах выше показаны только для демонстрации синтаксиса.
