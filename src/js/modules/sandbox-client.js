...
function hasDangerousBracketNotation(code) {
  ...
  return bracketPatterns.some(function (re) {
    return re.test(code);
  });
}
...
const CLIENT_VALIDATION_RULES = [
  ...
  {
    pattern:
      /\bos\b\s*\.\s*(?:system|popen|exec|remove|rmdir|rename|spawn|fork|kill|chdir|getcwd|listdir|walk|getenv|putenv|uname|getlogin|getpid|getuid|setuid)\s*\(/i,
    message: 'Опасный вызов os',
  },
  ...
  {
    pattern: /\b(?:__class__|__bases__|__mro__|__subclasses__|__globals__|__code__)\b/,
    message: 'Доступ к дандер-атрибутам через скобки',
  },
];
