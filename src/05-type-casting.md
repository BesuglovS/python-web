...
> **💡 Совет:** `input()` всегда возвращает строку. Если нужны вычисления — приводите к числу!
...
- `int(input())` без проверки — если пользователь введёт буквы, программа упадёт. Используйте `try/except` (будет в уроке 11).
